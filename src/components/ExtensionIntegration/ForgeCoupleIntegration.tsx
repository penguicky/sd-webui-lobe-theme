...
const ForgeCoupleIntegration = memo<ForgeCoupleIntegrationProps>(({
  ...
  const initializeShadowContainer = useCallback(async () => {
    ...
    try {
      ...
      console.log(`[ForgeCoupleIntegration] Initialized for ${mode}`);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      setError(errorMessage);
      console.error('[ForgeCoupleIntegration] Initialization failed:', err);
    }
  }, [mode]);
  ...
  const handleDimensionChange = useCallback((dimensions: { width: number; height: number }) => {
    if (eventBridgeRef.current && isInitialized) {
  ...
  useEffect(() => {
    ...
    const promptSelector = mode === 't2i' ? '#txt2img_prompt textarea' : '#img2img_prompt textarea';
