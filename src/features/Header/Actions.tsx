...
const CivitaiLogo = forwardRef<SVGSVGElement, { [key: string]: any, size?: string | number; }>((props, ref) => {
  const size = typeof props.size === 'string' ? parseInt(props.size) : (props.size || 16);
  return (
    <svg fill="currentColor" height={size} ref={ref} viewBox="0 0 16 16" width={size} {...props}>
...
const Actions = memo<ActionsProps>(() => {
  ...
  // Dynamic theme icon component
  const ThemeIcon = useMemo(() => createIconComponent(themeIcon, 16), [themeIcon]);
      ...
      !mobile ? (
        <>
          ...
          <a
            ...
            target="_blank"
          >
            <ActionIcon icon={createIconComponent('LayoutGrid', 16)} title="Cheat Sheet" />
          </a>
          <ActionIcon icon={createIconComponent('Github', 16)} onClick={handleOpenModal} title={feedbackTitle} />
        </>
      ) : null,
  ...
  return (
    <>
      <Space.Compact>
        ...
        <ActionIcon icon={createIconComponent('Settings', 16)} onClick={handleOpenSettings} title={settingTitle} />
      </Space.Compact>
